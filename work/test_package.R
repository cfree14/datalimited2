...
plot_dlm(output)
save(output, file="work/example_output/zbrt_output.Rdata")

# Test OCOM
output <- ocom(year=TIGERFLAT$yr, catch=TIGERFLAT$catch, m=0.27)
plot_dlm(output)
save(output, file="work/example_output/ocom_output.Rdata")
load(file="work/example_output/ocom_output.Rdata")

# Test BSM
output <- bsm(year=SOLIRIS$yr, catch=SOLIRIS$ct, biomass=SOLIRIS$bt, btype="CPUE", r.low=0.18, r.hi=1.02)
plot_dlm(output)
save(output, file="work/example_output/bsm_output.Rdata")

# Test cMSY
output <- cmsy2(year=SOLIRIS$yr, catch=SOLIRIS$ct, r.low=0.18, r.hi=1.02)
plot_dlm(output)
save(output, file="work/example_output/cmsy_output.Rdata")
