...
#' Plot cMSY model results
#'
#' Plots cMSY model results for following the example of Froese et al. (2016).
#' The following six plots are produced:
#' \itemize{
#'   \item{A - Catch time series}
#'   \item{B - Finding r-k pairs}
...
plot_cmsy2 <- function(output){
  ...
  lines(x=ref_ts$year, y=ref_ts$catch_ma, col="blue", lwd=1)
  ...
  points(x=rv.all, y=kv.all, pch=".", cex=4, col="gray")
  ...
  plot(x=rv.all, y=kv.all,
       pch=16, col="gray",log="xy", bty="l",
       xlab="r", ylab="k", main="C: Analysis of viable r-k")

  # Add cMSY r/k pair, with 95% CL lines
  points(x=r, y=k, pch=19, col="blue")
  lines(x=c(r_lo, r_hi), y=c(k, k), col="blue")
  lines(x=c(r, r), y=c(k_lo, k_hi), col="blue")
  ...
  plot(s ~ year, ref_ts, lwd=1.5, xlab="Year", ylab="Relative biomass B/k", type="l",
       ylim=c(0,ymax), bty="l", main="D: Biomass",col="blue")
  lines(x=ref_ts$year, y=ref_ts$s_lo, type="l", lty="dotted", col="blue")
  lines(x=ref_ts$year, y=ref_ts$s_hi, type="l", lty="dotted", col="blue")
  # Add lines for 0.5 and 0.25 biomass
  abline(h=0.5, lty="dashed")
  abline(h=0.25, lty="dotted")
  # plot biomass windows
  lines(x=c(start_yr, start_yr), y=startbio, col="blue")
  lines(x=c(int.yr, int.yr), y=intbio, col="blue")
  lines(x=c(end_yr, end_yr), y=endbio, col="blue")

  # E. Exploitation rate plot
  ##############################################################

  # Plot exploitation rate
  plot(er ~ year, ref_ts, type="l", bty="l", lwd=1.5, xlab="Year",
  ...
  points(x=ref_ts$s, y=cdivmsy, pch=16, col="blue")
